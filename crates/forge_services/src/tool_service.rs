...
use forge_domain::{
    Tool, ToolCallContext, ToolCallFull, ToolDefinition, ToolName, ToolResult, ToolResponseData, ToolService,
};
...
impl ToolService for ForgeToolService {
    async fn call(&self, context: ToolCallContext, call: ToolCallFull) -> ToolResult {
        ...
        let result = match output {
            Ok(output) => {
                // Check if output is already in front matter format
                if output.trim_start().starts_with("---") {
                    // Output might already be in front matter format, generated by the tool
                    // We'll just pass it through but still need to populate ToolResult fields
                    let mut tool_result = ToolResult::from(call);
                    tool_result.content = output;
                    tool_result
                } else {
                    // Generate a generic data structure for tools that don't provide structured data
                    let data = ToolResponseData::generic()
                        .with_metadata("tool_name", name.as_str());
                    ToolResult::from(call).with_frontmatter_response(data, output)
                }
            },
            Err(error) => {
                error!(error = ?error, "Tool call failed");
                // For errors, also use structured data
                let data = ToolResponseData::generic()
                    .with_metadata("tool_name", name.as_str())
                ...
                ToolResult::from(call)
                    .with_frontmatter_response(data, output)
                    .failure(error) // Also set is_error flag
            }
        };
